...
Toolset.CRED.shortcodeManager = function( $ ) {
	...
	self.initTemplates = function() {
		self.templates = {
			...
			credDelayed: {
				'cred-delete-relationship': wp.template( 'shortcode-cred-delete-relationship-template' )
			},
			afw: {
	...
	self.shortcodeDialogOpen = function( dialogData ) {
		...
		if ( _.has( self.templates.credDelayed, dialogData.shortcode ) ) {
		...
		$.ajax({
			...
			success:  function( originalResponse ) {
				...
				if ( response.success ) {
					...
					self.dialogs.shortcode.html( self.templates.credDelayed[ dialogData.shortcode ]( templateCompleteData ) );
					Toolset.hooks.doAction( 'cred-action-shortcode-dialog-loaded', dialogData );
					Toolset.hooks.doAction( 'toolset-action-shortcode-dialog-loaded', dialogData );
				} else {
