...
class CRED_PostExpiration {
	 ...
	 */
	public function cred_pe_meta_boxes( $metaboxes, $form_fields ) {
		global $post;
	 ...
	 */
	public function addCredPostExpireMetaBox( $form, $args ) {
			...
			foreach ( $posts_for_notifications as $post_meta ) {
				...
				foreach ( $post_meta->notifications as $key => $notification ) {
					...
					if ( $notification_time <= $now ) {
						...
						CRED_Notification_Manager_Post::get_instance()->send_notifications( $post_meta->post_id, $form_id, array( $notification ) );
	...
	public function extraBodyNotificationCodes( $codes, $form_id, $post_id ) {
		$extra_codes = array(
			...
			'%%POST_PARENT_TITLE%%' => $this->cred_pe_parent_info_by_post_id( $post_id, 'title' ),
			'%%POST_PARENT_LINK%%' => $this->cred_pe_parent_info_by_post_id( $post_id, 'url' ),
			'%%CRED_NL%%' => "\r\n",
		);
		foreach ( $extra_codes as $placeholder => $replace ) {
			if ( ! isset( $codes[ $placeholder ] ) ) {
