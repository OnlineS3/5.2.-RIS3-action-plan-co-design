...
final class CRED_Helper {
	...
	/**
	 * @deprecated Can be removed right away, not referenced anywhere
	 * @todo REMOVE
	 */
	...
	public static function accessNotActiveFormAlertNotice() {
        ...
        <p>
			<?php printf( __( 'This Form will be accessible to everyone, including guest (not logged in). They will be able to submit/edit content using this form.<br>To control who can use the form, please install %s', 'wp-cred' ), '<a target="_blank" href="https://toolset.com/home/toolset-components/#access">Access plugin</a>' ); ?>
			<?php
	...
	private static function buildCredCaps() {
		...
		if ( ! isset( $caps ) ) {
			...
			$forms = self::getAllFormsCached();
			// register custom Toolset Forms Frontend capabilities specific to each form type
			foreach ( $forms as $form ) {
	...
	/**
     * Add current Toolset Forms caps to specific role/user
     *
	 * @param $r
	 */
	private static function addCredCapsToRoleUser( $r ) {
		...
		// register custom Toolset Forms Frontend capabilities specific to each form type for existing forms
		// Add only new caps
	...
	public static function setupCustomUserCaps() {
		...
		function_exists( 'wpcf_access_register_caps' )
			...
			if ( $wp_roles->is_role( 'administrator' ) ) {
				...
				$administrator = false;
				trigger_error( __( 'Administrator Role not found! Toolset Forms Users capabilities will not work', 'wp-cred' ), E_USER_NOTICE );
	 ...
	 *
	 * Register a custom tab in the Access Control admin page for managing Toolset Forms capabilities.
	 *
	...
	/**
     * Register a new Types Access Area for custom Toolset Forms Frontend capabilities
     *
	 * @param $areas
	 *
	 * @return array
	...
	public static function register_access_cred_area( $areas ) {
		$CRED_ACCESS_AREA_NAME = __( 'Toolset Post Forms Frontend Access', 'wp-cred' );
	...
	public static function register_access_cred_user_area( $areas ) {
		$CRED_ACCESS_AREA_NAME = __( 'Toolset User Forms Frontend Access', 'wp-cred' );
	...
	/**
     * Register a new Types Access Group within Area for custom Toolset Forms Frontend capabilities
     *
	 * @param $groups
	 * @param $id
	 *
		...
		if ( $area_id == $CRED_ACCESS_AREA_ID && $group_id == $CRED_ACCESS_GROUP_ID ) {
			...
			foreach ( $forms as $form ) {
				...
				if ( $settings && isset( $settings->form['type'] ) && 'new' == $settings->form['type'] ) {
					...
					$caps[ $cred_cap ] = array(
						'cap_id' => $cred_cap,
						'title' => sprintf( __( 'Create Custom Post with the Toolset Form "%s"', 'wp-cred' ), $form->post_title ),
					...
					$caps[ $cred_cap ] = array(
						'cap_id' => $cred_cap,
						'title' => sprintf( __( 'Edit Own Custom Post with the Toolset Form "%s"', 'wp-cred' ), $form->post_title ),
						'default_role' => $default_role,
					);
					$cred_cap = 'edit_other_posts_with_cred_' . $form->ID;
					$caps[ $cred_cap ] = array(
						'cap_id' => $cred_cap,
						'title' => sprintf( __( 'Edit Others Custom Post with the Toolset Form "%s"', 'wp-cred' ), $form->post_title ),
						'default_role' => $default_role,
					);
				}
			}
			...
			$caps['delete_own_posts_with_cred'] = array(
				...
				'title' => __( 'Delete Own Posts using Toolset Forms', 'wp-cred' ),
				'default_role' => $default_role,
			);
			$caps['delete_other_posts_with_cred'] = array(
				'cap_id' => 'delete_other_posts_with_cred',
				'title' => __( 'Delete Others Posts using Toolset Forms', 'wp-cred' ),
				'default_role' => $default_role,
			);
		}
		...
		if ( $area_id == $CRED_ACCESS_AREA_ID && $group_id == $CRED_ACCESS_GROUP_ID ) {
			...
			foreach ( $forms as $form ) {
				...
				if ( $settings && isset( $settings->form['type'] ) && 'new' == $settings->form['type'] ) {
					...
					$caps[ $cred_cap ] = array(
						'cap_id' => $cred_cap,
						'title' => sprintf( __( 'Create Custom User with the Toolset Form "%s"', 'wp-cred' ), $form->post_title ),
						'default_role' => $default_role,
					);
				} elseif ( $settings && isset( $settings->form['type'] ) && 'edit' == $settings->form['type'] ) {
					$cred_cap = 'edit_own_user_with_cred_' . $form->ID;
					$caps[ $cred_cap ] = array(
						'cap_id' => $cred_cap,
						'title' => sprintf( __( 'Edit Own Custom User with the Toolset Form "%s"', 'wp-cred' ), $form->post_title ),
						'default_role' => $default_role,
					);
					$cred_cap = 'edit_other_users_with_cred_' . $form->ID;
					$caps[ $cred_cap ] = array(
						'cap_id' => $cred_cap,
						'title' => sprintf( __( 'Edit Others Custom User with the Toolset Form "%s"', 'wp-cred' ), $form->post_title ),
						'default_role' => $default_role,
					);
				}
			}
