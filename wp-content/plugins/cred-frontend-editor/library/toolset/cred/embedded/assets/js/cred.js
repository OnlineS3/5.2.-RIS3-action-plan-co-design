(function (window, $, settings, utils, gui, mvc, undefined) {
    ...
    var aux = {
        ...
        genScaffold: function () {
            ...
            var parents_out = '';
            if (!cred_settings.is_m2m_enabled
                && parseInt(response.parents_count, 10) > 0
            ) {
            ...
            var relationshipsOut = '';
            if (parseInt(response.relationships_count, 10) > 0) {
                for (var relationshipsField in response.relationships) {
                    if (response.relationships.hasOwnProperty(relationshipsField)) {
                        relationshipsOut += aux.relationshipOutput(response.relationships[relationshipsField], form_id_1, form_name_1, includeWPML, '') + NL + NL;
                    }
                }
            }

            var prfOut = '';
            if (parseInt(response.post_reference_fields_count, 10) > 0) {
                for (var postReferenceField in response.post_reference_fields) {
                    if (response.post_reference_fields.hasOwnProperty(postReferenceField)) {
                        prfOut += aux.relationshipOutput(response.post_reference_fields[postReferenceField], form_id_1, form_name_1, includeWPML, '') + NL + NL;
                    }
                }
            }
            ...
            if (parseInt(response.post_reference_fields_count, 10) > 0) {
                out += aux.groupOutputContent('post_reference_fields', 'Post Reference', prfOut, PAD) + NL + NL;
            }
            if (parseInt(response.relationships_count, 10) > 0) {
                out += aux.groupOutputContent('relationships', 'Relationships', relationshipsOut, PAD) + NL + NL;
            }
        ...
        genUserScaffold: function (referrer) {
            ...
            var relationships_out = '';
            if (parseInt(response.relationships_count, 10) > 0) {
                for (var relationships_field in response.relationships) {
                    if (response.relationships.hasOwnProperty(relationships_field)) {
                        relationships_out += aux.relationshipOutput(response.relationships[relationships_field], form_id_1, form_name_1, includeWPML, '') + NL + NL;
                    }
                }
            }
            ...
            if (parseInt(response.relationships_count, 10) > 0) {
                out += aux.groupOutputContent('relationships', 'Relationships', relationships_out, PAD) + NL + NL;
            }
        ...
        onLoad: function (response) {
            ...
            if (response.post_fields && parseInt(response.post_fields_count) > 0) {
                ...
                _.each( post_response, function( value, response_field, list ) {
                    if ( _.has( value, 'supports' ) && ! value.supports ) {
                        return;
                    }
                    data = jQuery("<a href='javascript:;' class='button cred_field_add' title='" + post_response[response_field].description + "'>" + post_response[response_field].name + "</a>");
                    data.data('field', post_response[response_field]);
            ...
            if (
                response.hierarchical_parents
                && ! _.isEmpty( response.hierarchical_parents )
            ) {
                content_item = jQuery('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.hierarchical_parent_fields + '</a></div>');
                content_item_inside = jQuery('<div class="cred-accordeon-item-inside"></div>');
                content_item.append(content_item_inside);
                box_content.append(content_item);
                post_response = response.hierarchical_parents;
                _.each( post_response, function( value, response_field, list ) {
                    data = jQuery("<a href='javascript:;' class='button cred_field_add' title='" + post_response[response_field].description + "'>" + post_response[response_field].name + "</a>");
                    data.data('field', post_response[response_field]);
                    content_item_inside.append(data);
            ...
            if (response.relationships && parseInt(response.relationships_count) > 0) {
                content_item = jQuery('<div class="cred-accordeon-item"><a href="javascript:;" class="cred-fields-group-heading">' + cred_settings.locale.relationship_fields + '</a></div>');
                content_item_inside = jQuery('<div class="cred-accordeon-item-inside"></div>');
                content_item.append(content_item_inside);
                box_content.append(content_item);
                post_response = response.relationships;
                for (var response_field in post_response) {
                    if (post_response.hasOwnProperty(response_field)) {
                        data = jQuery("<a href='javascript:;' class='button cred_field_add' title='" + post_response[response_field].description + "'>" + post_response[response_field].name + "</a>");
                        data.data('field', post_response[response_field]);
            ...
            if (response.extra_fields && parseInt(response.extra_fields_count) > 0) {
                ...
                _.each( post_response, function( value, response_field, list ) {
                    ...
                    if ( _.has( value, 'disabled' ) && value.disabled ) {
        ...
        shortcode: function (field, extra) {
            ...
            if (field && field.slug) {
                if (field.post_type) {
                    //@deprecated since 2.0 with m2m feature      post attribute is not needed and used anymore
        ...
        relationshipShortcode: function (field) {
            var required = "";
            if (field.required && field.required === true) {
                required = " required='true'";
            }
            var select_text = "";
            if ( field.is_post_reference
                || ( field.cardinality[field.role].max === 0
                    || field.cardinality[field.role].max === 1 )
            ) {
                select_text += " select_text='" + cred_settings.locale.default_select_text + "'";
            }
            var field_out = "[cred_field field='" + field.key + "'" + required + select_text + " class='form-control' output='bootstrap']";
            return field_out;
        },
        ...
        relationshipOutput: function (relationship, form_id, form_name, WPML, pad) {
            WPML = WPML || false;
            if (!pad)
                pad = '';
            var relationship_out = [];
            relationship_out.push('<div class="form-group">');

            if (WPML) {
                relationship_out.push(pad + PAD + "<label>[wpml-string context='" + cred_settings._current_page + "-" + form_name + "-" + form_id + "' name='" + relationship.label + "']" + relationship.label + "[/wpml-string]</label>");
            } else {
                relationship_out.push(pad + PAD + '<label>' + relationship.label + '</label>');
            }
            relationship_out.push(pad + PAD + aux.relationshipShortcode(relationship));
            relationship_out.push(pad + '</div>');
            return relationship_out.join(NL);
        },
    ...
    var self = {
        ...
        doCheck: function (step) {
            ...
            var is_form_settings = ((!step || (step && step == 3)) && jQuery('input[name="_cred[form][type]"]').is(':visible'));
            var type_form = aux.getFormType();
            ...
            var is_user_form = ((!step || (step && step == 3)) && aux.isUserForm());
        ...
        forms: function () {
            ...
            _credView = new mvc.View('cred', _credModel, {
                init: function () {
                        ...
                        .event('model:change', function (e, data) {
                            ...
                            if ('[post][post_type]' == data.key) {
                                ...
                                if (cred_settings._current_page == 'cred-user-form') {
                                    ...
                                    if ( _.has( cache.posts, data.value ) ) {
                                        aux.onLoad( cache.posts[ data.value ] );
                    ...
                    utils.attach('cred.fieldsLoaded', adjustGuiForRfg);
                    ...
                    $cred_shortcodes_box.on('click', 'a.cred_field_add', function (e) {
                        ...
                        // fields can have, placeholder, readonly and escape attributes
                            ...
                            return false;
                        } else if (data.is_relationship) {
                            shortcode = aux.relationshipShortcode(data);
                        } else {
                            shortcode = aux.shortcode(data);
                        }
                        utils.InsertAtCursor(jQuery('#content'), shortcode);
