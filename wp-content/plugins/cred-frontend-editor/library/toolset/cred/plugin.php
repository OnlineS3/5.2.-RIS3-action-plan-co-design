...
function toolset_cred_plugin_plugin_row_meta($plugin_meta, $plugin_file, $plugin_data, $status) {
    ...
    if ($plugin_file == $this_plugin) {
        ...
        $plugin_meta[] = sprintf(
                '<a href="%s" target="_blank">%s</a>', 'https://toolset.com/version/cred-' . $ver2url . '/?utm_source=credplugin&utm_campaign=cred&utm_medium=release-notes-plugin-row&utm_term=CRED ' . CRED_FE_VERSION . ' release notes', __('Toolset Forms ' . CRED_FE_VERSION . ' release notes', 'wp-cred')
        );
    }
    return $plugin_meta;
}
    ...
    CRED_Loader::add('dependencies', array(
        ...
        'MODEL' => array(
            '%%PARENT%%' => array(
	            ...
	            array(
		            ...
		            'path' => CRED_MODELS_PATH . '/CRED_Fields_Types_Utils.php'
	            )
            ),
            'Forms' => array(
                // dependencies
        ...
        'TEMPLATE' => array(
	        ...
	        'how-to-display-meta-box' => array(
		        'path' => CRED_TEMPLATES_PATH . "/how-to-display-meta-box.tpl.php"
	        ),
