...
class CRED_Frontend_Select2_Manager {
	...
	public function get_potential_relationship_parents() {
		...
		$mandatory_parameters = array( 'post_id', 'slug', 'role' );
		...
		$referrer_role_post_id = (int) $_POST[ 'post_id' ];
		...
		//Query Call
		$potential_associable_parent_query = new CRED_Form_Potential_Associable_Parent_Query();
		$potential_parents = $potential_associable_parent_query->get_potential_associable_parent_result( $relationship_definition, $role_object, $connect_to_as_object, $args );
	 ...
	 *
	 * @param WP_Post[]|Toolset_Post[] $wp_items
	 *
	 * @throws Exception
	 */
		...
		if ( is_array( $wp_items ) && ! empty( $wp_items ) ) {
			foreach ( $wp_items as $item ) {
				if ( $item instanceof IToolset_Post ) {
					$output_results[] = array(
						'text' => $item->get_id() . " " . $item->get_title(),
						'id' => $item->get_id(),
					);
